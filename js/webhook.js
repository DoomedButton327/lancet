  ...
  async function notify({ action, sampleName, personName, details }) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Discord webhook rejected:', res.status, bodyText);
      }
    } catch (e) {
      // Network-level failure (blocked, offline, bad URL, etc) — logged loudly
      // so it's diagnosable, but never breaks the rest of the app.
